...
par=default_par(); tr=simulate_optimal(par,13,2601);
figure('Color','w','Position',[100 100 900 560]); hold on;
plot(tr.t,tr.q,'LineWidth',2.5,'DisplayName','Isolation q^*(t)');
plot(tr.t,tr.beta1/par.c,'LineWidth',2.0,'DisplayName','beta_1^*/c');
plot(tr.t,tr.beta2/par.c,'LineWidth',2.0,'DisplayName','beta_2^*/c');
plot([0 13],[par.p par.p],'--','LineWidth',1.2,'DisplayName','Natural normalized rate p');
xlabel('Time'); ylabel('Control / normalized coefficient');
title('One control produces two different optimal coefficients'); ylim([-0.02 1.05]); grid on; box on; legend('Location','northeast');
out=fullfile(root,'figures','Figure_3_control_coefficients.jpg'); print(gcf,out,'-djpeg','-r300'); fprintf('%s\n',out);
