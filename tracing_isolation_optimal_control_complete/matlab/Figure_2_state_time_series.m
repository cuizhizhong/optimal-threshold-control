...
par=default_par(); tr=simulate_optimal(par,18,3001); sol=tr.summary;
figure('Color','w','Position',[100 100 900 560]); hold on;
plot(tr.t,tr.s,'LineWidth',2.2,'DisplayName','s(t)'); plot(tr.t,tr.i,'LineWidth',2.6,'DisplayName','i(t)');
plot([0 18],[par.K par.K],'--','LineWidth',1.5,'DisplayName','Capacity K');
times=[sol.tau1,sol.tau1+sol.tau_boundary,sol.tau1+sol.tau_boundary+sol.tau_q1];
for k=1:3, xline(times(k),':','LineWidth',1.2,'HandleVisibility','off'); end
text(times,[0.94 0.94 0.94],{'tau_1','tau_B','tau_R'},'Units','data','FontSize',9);
xlabel('Time'); ylabel('Population fraction'); title('State trajectories under the optimal policy');
ylim([0 1.02]); grid on; box on; legend('Location','northeast');
out=fullfile(root,'figures','Figure_2_state_time_series.jpg'); print(gcf,out,'-djpeg','-r300'); fprintf('%s\n',out);
