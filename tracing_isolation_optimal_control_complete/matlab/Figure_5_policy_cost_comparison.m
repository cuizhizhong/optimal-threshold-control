...
par=default_par(); sol=optimize_constant_c(par); s1=sol.s1;
fullCap=q1_cost(s1,par.K,par); sEarly=fzero(@(s) natural_i(s,par)-0.10,[par.h,par.s0]); fullEarly=q1_cost(sEarly,0.10,par);
vals=[sol.cost,sol.fill_box_cost,fullCap,fullEarly]; labs={'Optimal','Pure fill-box','Full isolation at K','Full isolation at i=0.10'};
figure('Color','w','Position',[100 100 930 560]); b=bar(vals); grid on; box on;
set(gca,'XTick',1:4,'XTickLabel',labs,'XTickLabelRotation',15); ylabel('Total tracing cost'); title('Cost comparison of feasible policies');
for k=1:4, text(k,vals(k)+0.03,sprintf('%.3f',vals(k)),'HorizontalAlignment','center','FontSize',9); end
out=fullfile(root,'figures','Figure_5_policy_cost_comparison.jpg'); print(gcf,out,'-djpeg','-r300'); fprintf('%s\n',out);
