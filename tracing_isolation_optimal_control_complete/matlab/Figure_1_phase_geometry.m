...
par=default_par(); sol=optimize_constant_c(par); tr=simulate_optimal(par,14,2401);
sgrid=linspace(par.h,par.s0,700); lf=natural_i(sgrid,par); safe=safe_i(sgrid,par);
[swS,swI,~,~]=switching_curve(par,par.s0,240);
figure('Color','w','Position',[100 100 900 620]); hold on;
plot(sgrid,lf,'--','LineWidth',1.8,'DisplayName','Uncontrolled orbit');
plot(sgrid,safe,':','LineWidth',2.0,'DisplayName','Maximal safe orbit');
if ~isempty(swS), [swS,ord]=sort(swS); swI=swI(ord); plot(swS,swI,'LineWidth',2.0,'DisplayName','Switching curve'); end
plot([0.08 1.01],[par.K par.K],'-.','LineWidth',1.5,'DisplayName','Capacity');
plot([par.h par.h],[0 0.38],':','LineWidth',1.2,'DisplayName','s=h');
plot(tr.s,tr.i,'LineWidth',3.0,'DisplayName','Optimal trajectory');
pts=[par.s0 par.i0; sol.s1 par.K; sol.s_switch sol.i_switch; sol.s_release sol.i_release];
scatter(pts(:,1),pts(:,2),45,'filled','HandleVisibility','off');
text(pts(:,1)+0.008,pts(:,2)+0.008,{'initial','first hit','switch to q=1','release'},'FontSize',9);
xlim([0.08 1.01]); ylim([0 max(0.38,1.08*max(lf))]); grid on; box on;
xlabel('Susceptible fraction s'); ylabel('Infectious fraction i');
title('Phase-plane geometry of the tracing-isolation optimum'); legend('Location','northwest','NumColumns',2);
out=fullfile(root,'figures','Figure_1_phase_geometry.jpg'); print(gcf,out,'-djpeg','-r300'); fprintf('%s\n',out);
