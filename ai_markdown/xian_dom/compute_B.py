"""Heavy compute for Panel B; saves everything to /tmp/panelB.pkl so plotting is fast."""
...
with open("/tmp/panelB.pkl", "wb") as f:
    pickle.dump(out, f)
open("/tmp/panelB.DONE", "w").write("ok")
print(f"SAVED /tmp/panelB.pkl  total {time.time()-t0:.0f}s")
