...
\begin{itemize}
  ...
  \item $q_0$ 同时经前、后两段，但两通道反向：$q_0$ 增大使后段因子减小（推 $\lambda$ 增），却使 $S^*$ 及前段因子下降（推 $\lambda$ 减；$\bar S$ 不含 $q_0$），故净方向一般不单调。数值上，第~\ref{sec:num}节参数下 $\lambda$ 在 $q_0\lesssim0.3$ 上随 $q_0$ 增大（净向 $t_2$），但在整个可行区间上非单调；在较低 $\beta$（如 $\beta=0.10$）背景下则先增后减。
\end{itemize}
...
$h=h(\theta,N)$。$h$ 关于 $\theta$ 单调递增（数值上 $h\approx0.18$）。若再要求
...
的西安全市重拟合值 $\bar I_{\rm peak}^{\rm T}=151.90$、$\bar J^{\rm T}=49.35$、$\bar I_{t\rm cum}^{\rm T}=2096.76$
（近似锚定，见局限）。由式~\eqref{eq:s1:Imax-no} 得 $i_{\max}^{no}\approx0.1047$。数值求解~\eqref{eq:dom:thetadefs}：
\[
